...
//
// Modified by Sam Marshall on 05/15/2013
//
...
//#define NSLog(m, ...) NSLog(@"[%s:%d] %s: " m, __FILE__, __LINE__, __PRETTY_FUNCTION__, ## __VA_ARGS__);
...
mach_error_t AMDeviceNotificationSubscribe(am_device_notification_callback callback, uint32_t unused0, uint32_t unused1, void *callback_data, am_device_notification *notification);
...
- (NSData *)getFileHash:(NSString *)path {
	...
	if ([self ensureConnectionIsOpen]) {
		void *op = AFCOperationCreateGetFileHash(NULL, (CFStringRef)path, self);
		if (op) {
...
- (BOOL)fileExistsAtPath:(NSString *)path isDirectory:(BOOL *)isDirectory {
	...
	if ([self ensureConnectionIsOpen]) {
		...
		if (result && [self checkStatus:AFCFileInfoOpen(_afc, [path UTF8String], &dict) from:"AFCFileInfoOpen"]) {
			NSMutableDictionary *fileInfo = [self readAfcDictionary:dict];
			AFCKeyValueClose(dict);
			*isDirectory = [[fileInfo objectForKey:@"st_ifmt"] isEqualToString:@"S_IFDIR"];
		}
	}
	return result;
}
...
- (BOOL)processOp:(afc_operation)op from:(const char *)func
{
	...
	if ([self checkStatus:AFCConnectionProcessOperation(_afc, op, AFCConnectionGetIOTimeout(_afc)) from:"AFCConnectionProcessOperation"]) {
		afc_error_t e = AFCOperationGetResultStatus(op);
		CFShow(AFCOperationGetResultObject(op));
		if (e == 0) {
			[self clearLastError];
			result = YES;
		} else {
...
- (id)initWithAMDevice:(AMDevice*)device {
	self = [super initWithName:@"com.apple.afc" onDevice:device];
	if (self) {
...
- (id)initWithAMDevice:(AMDevice*)device {
	self = [super initWithName:@"com.apple.crashreportcopymobile" onDevice:device];
	if (self) {
		int ret = AFCConnectionOpen(_service, 0/*timeout*/, &_afc);
		if (ret != 0) {
			NSLog(@"AFCConnectionOpen failed: %d", ret);
...
- (id)initWithAMDevice:(AMDevice*)device {
	self = [super initWithName:@"com.apple.afc2" onDevice:device];
	if (self) {
		int ret = AFCConnectionOpen(_service, 0/*timeout*/, &_afc);
		if (ret != 0) {
			NSLog(@"AFCConnectionOpen failed: %d", ret);
...
- (id)initWithAMDevice:(AMDevice*)device {
	self = [super initWithName:@"com.apple.mobile.installation_proxy" onDevice:device];
	if (self) {
	}
	return self;
}
...
- (id)initWithAMDevice:(AMDevice*)device listener:(id)listener message:(SEL)message {
	self = [super initWithName:@"com.apple.syslog_relay" onDevice:device];
	if (self) {
		_listener = listener;
		_message = message;
		int sock = (int)((uint32_t)_service);
...
- (id)initWithAMDevice:(AMDevice*)device {
	self = [super initWithName:@"com.apple.mobile.file_relay" onDevice:device];
	if (self) {
		_used = NO;
	}
	return self;
...
- (id)initWithAMDevice:(AMDevice*)device {
	self = [super initWithName:@"com.apple.mobile.notification_proxy" onDevice:device];
	if (self) {
...
- (id)initWithAMDevice:(AMDevice*)device {
	self = [super initWithName:@"com.apple.springboardservices" onDevice:device];
	if (self) {
		// nothing special
	}
	return self;
...
- (id)initWithAMDevice:(AMDevice*)device {
	self = [super initWithName:@"com.apple.mobilesync" onDevice:device];
	if (self) {
